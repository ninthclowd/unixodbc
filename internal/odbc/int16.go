...
func (c *columnInt16) Value() (driver.Value, error) {
	...
	if _, err := c.result(c.api().SQLGetData(api.SQLHSTMT(c.hnd()), c.columnNumber, api.SQL_C_SSHORT, api.SQLPOINTER(&value), 0, &valueLength)); err != nil {
		return nil, err
	}
...
func (s *Statement) bindInt16(index int, value int16) error {
	_, err := s.result(s.api().SQLBindParameter((api.SQLHSTMT)(s.hnd()), api.SQLUSMALLINT(index+1), api.SQL_PARAM_INPUT,
		api.SQL_C_SSHORT, api.SQL_SMALLINT,
		0, 0,
