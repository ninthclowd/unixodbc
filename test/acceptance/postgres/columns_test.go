...
func TestColumnBinding(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			sql:                  `SELECT CAST('2009-05-18' AS DATE) AS "COL_DATE"`,
		...
		{
			description:          "it should decode a TIMESTAMP value",
			sql:                  `SELECT CAST('2009-05-18 12:03:05' AS TIMESTAMP) AS "COL_TIMESTAMP"`,
			wantColumnName:       "COL_TIMESTAMP",
			wantTypeName:         "COL_TIMESTAMP",
			wantDatabaseTypeName: "",
			wantNullableOk:       true,
			wantNullable:         true,
			wantDecimalOk:        false,
		...
		{
			description:          "it should decode a numeric value",
			sql:                  `SELECT CAST(12.34 as numeric(10,2)) AS "COL_DEC"`,
		...
		{
			description:          "it should decode a numeric value",
			sql:                  `SELECT CAST(543515121.33 as numeric) AS "COL_DOUBLE"`,
		...
		{
			description:          "it should decode a BYTEA value",
			sql:                  `SELECT CAST('BIN' as BYTEA) AS "COL_BINARY"`,
