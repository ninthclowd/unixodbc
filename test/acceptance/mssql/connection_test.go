...
func newTestConnection(t *testing.T) (db *sql.DB, conn *sql.Conn, ctx context.Context, finish func()) {
	...
	db, err := sql.Open("unixodbc", fmt.Sprintf("Driver=FreeTDS;Server=%s;Port=1433;Database=tempdb;PWD=Acceptance!;UID=SA", host))
	if err != nil {
...
func TestConnection_ExecContext_Cancel(t *testing.T) {
	...
	start := time.Now()
	_, err := conn.ExecContext(execCtx, "WAITFOR DELAY '00:00:02'")
	elapsed := time.Since(start)
	if elapsed.Seconds() > 1 {
		t.Errorf("query did not return immediately when cancelled")
	}

	if err == nil || !strings.Contains(err.Error(), "[HY008:0]") {
...
func TestConnection_QueryContext_Cancel(t *testing.T) {
	...
	start := time.Now()
	_, err := conn.QueryContext(execCtx, "WAITFOR DELAY '00:00:02'")
	elapsed := time.Since(start)
	if elapsed.Seconds() > 1 {
		t.Errorf("query did not return immediately when cancelled")
	}
